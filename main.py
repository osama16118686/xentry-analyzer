...
def help_command(message):
    help_text = (
        ...
        "/price <رمز العملة> – عرض السعر الحالي للعملة\n"
...
# إضافة أمر /price الجديد
@bot.message_handler(commands=['price'])
def price_command(message):
    try:
        parts = message.text.split()
        if len(parts) != 2:
            bot.send_message(message.chat.id, "❌ الرجاء استخدام الأمر بهذا الشكل:\n/price BTC")
            return
        symbol = parts[1].lower()
        price = fetch_current_price(symbol)
        if price is not None:
            bot.send_message(message.chat.id, f"💰 سعر {symbol.upper()} الحالي: ${price}")
        else:
            bot.send_message(message.chat.id, f"❌ لم أتمكن من العثور على سعر العملة {symbol.upper()}.")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ خطأ أثناء جلب السعر: {e}")
...
def fetch_current_price(symbol):
    try:
        ...
        # تحويل symbol إلى id صحيح لـ CoinGecko
        # هذه قائمة مختصرة لبعض الرموز المشهورة لتحويلها إلى ids معروفة
        symbol_to_id = {
            "btc": "bitcoin",
            "eth": "ethereum",
            "xrp": "ripple",
